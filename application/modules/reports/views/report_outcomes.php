...
<?php include('includes/outcomes_context_menu.php'); ?>
    ...
    <thead>
       ...
       <?php 
         ...
         foreach($objectives as $objective):
          $outcomes = outcomes($objective->id,true);
          if(count($outcomes)>0):
        ?>
             <tr>
            ...
            <?php 
                  ...
                  foreach($outcomes as $outcome):
                    $indicatorData = indicators($outcome->id,true);
                 ...
                 ?>
                    ...
                    <?php foreach($indicatorData as $param):

                        $value = param_data($param->id,true);
                        $meetsTarget = ( $value && (!empty($value->target_value)) && is_numeric($value->target_value) && $value->indicator_value>=$param->target_value)?true:false;
                     ?>
                        <tr style="background-color:<?=(($rows%2)>0)?'#eee':''?>"> 
                            <td> <?=$param->indicator_name?></td>
                            <td> <?=($param->target_value)?$param->target_value:''?></td>
                            <td  style="background-color:<?=($meetsTarget)?'#3bc92e':'#f79797'?>"> 
                                <?=($value)?$value->indicator_value:'N/A'?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
